<?php namespace Nemerki\API\Classes;
...
abstract class Transformer extends TransformerAbstract
{
    ...
    protected function img($file, array $customSizes = [], $replace = false)
    {
        if (!$file)
            return null;
        $img = new Image('/storage/app/media' . $file);
        $image = [
            'original' => '' . $img->resize(),
        ];
        ...
        foreach ($sizes as $name => $size) {
            $image[$name] = '' . call_user_func_array([$img, 'resize'], $size);
